...
class DoctorSchedule extends Component {
    ...
    async componentDidMount() {
        ...
        // // console.log('Tiếng anh: ', moment(new Date()).locale('en').format("ddd - DD/MM"));

        // console.log('huynh check alldays: ', allDays)
    ...
    getArrDays = (language) => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (language === LANGUAGES.VI) {
                ...
                //c1:  capitalizeFirstLetter
                // object.label = moment(new Date()).add(i, 'days').format('dddd - DD/MM').replace(/^t/g, 'T').replace('chủ nhật', 'Chủ nhật');

                //c2:  capitalizeFirstLetter
                if (i === 0) {
                    let ddMM = moment(new Date()).format('DD/MM');
                    let today = `Hôm nay - ${ddMM}`;
                    object.label = today;
                } else {
                    let firstLetter = moment(new Date()).add(i, 'days').format('dddd - DD/MM');
                    object.label = this.capitalizeFirstLetter(firstLetter);
                }
            }
            else {
                if (i === 0) {
    ...
    handleClickScheduleTime = (time) => {
        ...
        console.log('Huynh check time: ', time)
    }
